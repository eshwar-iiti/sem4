const courseMarksConfig = {
    "CS 210": {
        ...
        components: [
            { id: "q1", label: "Quiz 1 (10%)" },
            { id: "q2", label: "Quiz 2 (10%)" },
            { id: "q3", label: "Quiz 3 (10%)" },
            { id: "q4", label: "Quiz 4 (10%)" },
            { id: "mid", label: "Mid Sem (25%)" },
            { id: "end", label: "End Sem (35%)" },
            { id: "surprise", label: "Surprise Quiz (10%)" }
        ],
        note: "Best 3 quizzes are counted"
    },
    "CS 202": {
...
function toggleCourseDetails(courseCode) {
    const safeCode = courseCode.replace(/\s+/g, '-');
    const detailsDiv = document.getElementById(`details-${safeCode}`);
...
function renderCourseMarks() {
    for (const [code, config] of Object.entries(courseMarksConfig)) {
        ...
        if (config.note)
            html += `<p style="font-size:12px;color:#777">${config.note}</p>`;
